...
time.sleep(3)
ps_output = [x for x in os.popen('ps -ef | grep \&\&\ python3\ runner_usl.py\ ' + subreddit_name).read().splitlines() if 'grep' not in x]
...
if len(ps_output) == 1:
        ...
        os.system("kill $(ps aux | grep '[p]ython3 runner_usl.py\ " + subreddit_name + "' | awk '{print $2}')")
